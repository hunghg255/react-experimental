...
a.report({node:F,message:da})}}}catch(l){Z.e(l)}finally{Z.f()}}}}catch(l){m.e(l)}finally{m.f()}}},t.CallExpression=function(h){if(na(h.callee)){var z=q[q.length-1],K=v[v.length-1],x=z.get(K);x||(x=[],z.set(K,x));x.push(h.callee)}"Identifier"===h.callee.type&&("useEffect"===h.callee.name||xa(h.callee))&&0<h.arguments.length&&(f=h)},t.Identifier=function(h){null==f&&G.has(h)&&"CallExpression"!==h.parent.type&&a.report({node:h,message:"`"+a.getSource(h)+'` is a function created with React Hook "useEffectEvent", and can only be called from the same component. They cannot be assigned to variables or passed down.'})},
